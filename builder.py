...
class Builder:
    ...
    @staticmethod
    def _pyinstaller_build(source_name, source_folder, working_dir, project_name, build_target, includes_folder, build_options='', buttons_to_disable=None):
        try:
    ...
    @staticmethod
    def pyinstaller_build(source_name, source_folder, working_dir, project_name, build_target, includes_folder, build_options='', buttons_to_disable=None):
        ...
        t = threading.Thread(target=Builder._pyinstaller_build, args=(source_name, source_folder, working_dir, project_name, 
                                                                     build_target, includes_folder, build_options, buttons_to_disable))
        t.start()
        
        
        
    @staticmethod
    def _cxfreeze_build(source_name, source_folder, working_dir, project_name, build_target, includes_folder, setup_file, buttons_to_disable=None):
        try:
            for button in buttons_to_disable:
                button.setDisabled(True)
            #Создаем временную папку по пути working_dir и компилируем в ней проект
    ...
    @staticmethod            
    def cxfreeze_build(source_name, source_folder, working_dir, project_name, build_target, includes_folder, setup_file, buttons_to_disable=None):
        #Запускаем метод в новом потоке чтобы не зависло главное окно с интерфейсом(на случай если такое имеется)
        t2 = threading.Thread(target=Builder._cxfreeze_build, args=(source_name, source_folder, working_dir, project_name, 
                                                                     build_target, includes_folder, setup_file, buttons_to_disable))
        t2.start()
